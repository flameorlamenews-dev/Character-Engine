...
function boostToY(boostCut: number): number {
  // Positive boost = higher on screen = lower Y value
...
export function EmotionEQDetail({
  ...
  // Build the EQ curve path through all trait points
  // Start at the left edge at zero (no modification), curve through each point, end at right edge at zero
  ...
  // Build smooth SVG path through these points
  ...
  // Build filled area from curve down/up to center line
  const filledPath = curvePath +
    ` L ${curvePoints[curvePoints.length - 1].x} ${CENTER_Y}` +
    ` L ${curvePoints[0].x} ${CENTER_Y} Z`;

  // Also build the original emotion line (smaller, for reference)
  ...
  return (
    <svg width={totalWidth} height={EQ_DETAIL_HEIGHT} className="block">
      ...
      {/* Grid lines — vertical (chapter markers) */}
      {Array.from({ length: totalChapters }, (_, i) => {
        const x = chapterToX(i, zoomLevel);
        return (
          <line
            key={i}
            x1={x} y1={0} x2={x} y2={EQ_DETAIL_HEIGHT}
            stroke="#1a1a30" strokeWidth={0.5}
          />
        );
      })}
      ...
      {/* Reference: original emotion line (dimmed) */}
      {emotionLinePath && (
        <path
          d={emotionLinePath}
          fill="none"
      ...
      {/* Trait control points — the draggable circles */}
      {sorted.map((point) => {
        const x = posToX(point.chapterPosition, zoomLevel);
        const y = boostToY(point.boostCut);
