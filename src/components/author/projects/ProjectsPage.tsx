...
export function ProjectsPage({ userId, onSelectProject }: ProjectsPageProps) {
  ...
  const handleDelete = async (project: ProjectWithCounts) => {
    ...
    // specific row's button on the next render — a fast double-click on the
    // same row, or a click on row B mid-delete of row A, would otherwise
    // both fire handleDelete in parallel.
    ...
    try {
      // Atomic server-side cascade via migration 012. The previous N-step
      // client-side cleanup raced with in-flight analysis writes (a new
      // character_timeline_entries row landing between the cleanup and the
      // project DELETE would 23503-block the cascade) and was vulnerable to
      // permissive-RLS quirks where .delete().in() reported success while
      // affecting zero rows. The RPC runs the whole sequence in one
      // transaction with an auth.uid() ownership check.
