...
import { BookOpen, Users, LogOut, BookText, Loader2, CheckCircle, AlertCircle } from "lucide-react";
...
const Dashboard = ({ session, projectId }: { session: any; projectId: string }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const basePath = `/projects/${projectId}`;
  const activeView = useMemo(() => {
    if (location.pathname.endsWith("/characters")) return "characters";
    if (location.pathname.endsWith("/glossary")) return "glossary";
    return "manuscripts";
  }, [location.pathname]);
  ...
  const fetchManuscriptStatuses = useCallback(async () => {
    ...
    if (data) {
      ...
      if (analyzingManuscriptId) {
        const ms = data.find(m => m.id === analyzingManuscriptId);
  ...
  }, [session.user.id, analyzingManuscriptId]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 bg-muted p-1 rounded-lg">
              <Button
                ...
                onClick={() => navigate(`${basePath}/manuscripts`)}
              ...
              <Button
                ...
                onClick={() => navigate(`${basePath}/characters`)}
                className="gap-2 font-semibold"
                size="lg"
              >
              ...
              <Button
                ...
                onClick={() => navigate(`${basePath}/glossary`)}
                className="gap-2 font-semibold"
                size="lg"
              >
